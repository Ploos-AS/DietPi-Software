...
dps_check_compatibility() {
	...
	local codenames=${DPS_DEBIAN_CODENAMES:-}
	...
	local arch codename ram disk cmd
	...
	[[ -z $codenames ]] || [[ " $codenames " == *" $codename "* ]] ||
		dps_die "Unsupported Debian release: $codename (supported: $codenames)"
	...
	for cmd in $required_commands; do
		dps_has_command "$cmd" || dps_die "Required command is missing: $cmd"
	done
