...
grep -q 'Ports below 1024 require an explicit privileged-port setup' "$ITEM/install"
grep -q 'Existing .* does not contain a valid Port setting' "$ITEM/install"
grep -q "apt-get install -y endlessh iproute2" "$ITEM/install"
